$(function() {
    ...
    document.getElementById('searchPermissions').addEventListener('input', function() {
        ...
        rows.forEach(row => {
            ...
            const permissions = row.querySelector('td:nth-child(3)').textContent.toLowerCase();
            if (groupName.includes(searchValue) || permissions.includes(searchValue)) {
                row.style.display = '';
            } else {
...
$(document).ready(function() {
    //Only for edit operation
    if($('input[name="_method"]').val().toUpperCase() == 'PUT' ){
        // Loop through each permission and check the corresponding checkbox
        allocatedPermissions.forEach(function(permission) {
            let checkbox = $('[id="' + permission + '"]');
