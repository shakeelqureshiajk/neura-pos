$(function() {
    ...
    $(document).ready(function() {
        showCompanyCurrencySelection();
    });

    /**
     * Show or Hide Warehouse Selection
     * */
    $(document).on('change', 'input[name="is_company_currency"]', function() {
        showCompanyCurrencySelection();
    });

    function showCompanyCurrencySelection() {
        var isCompanyCurrency = $('input[name="is_company_currency"]:checked').val();
        var btn = '';
        if(isCompanyCurrency){
            $("input[name='exchange_rate']").val('1.00').prop('readonly', true).addClass('cursor-not-allowed');
        }else{
            $("input[name='exchange_rate']").prop('readonly', false).removeClass('cursor-not-allowed');
        }
    }
});
